...
いちばん早い方法：Macの譜面工房の画面へMP3・M4A・WAVをドラッグ＆ドロップするだけです。3難易度の譜面を作り、このMacのゲーム（Macのブラウザの中）にもすぐ追加します。曲名・アーティストは音源のタグ（なければファイル名）を使います。

- 1曲だけなら `曲名_ちゃちゃまる.zip` を保存します。
- 何曲かまとめてドロップすると、最後に1つのZIP（`ちゃちゃまる曲パック_N曲_日時.zip`）にまとめて保存します。前に作った曲も「作った曲をまとめてiPhoneへ」で選んで1つのZIPにできます。
- ZIPを Files / iCloud / AirDrop などで自分のiPhoneへ移し、ゲームの「曲を追加」で選びます。まとめたZIP（や複数のZIP）なら、まだ入っていない曲だけを追加し、同じ曲はそのまま、作り直した版は新しい方に入れ替えて、結果（追加・更新・すでにある）を表示します。
