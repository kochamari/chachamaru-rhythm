...
test('V02 independently animated head, tail, paws and scarf during real chorus playback',async({page},info)=>{
 ...
 // The in-game puppet is drawn with WebGL; sample the bone rotations the
 // production renderer applies each frame.
 const frames=await page.evaluate(async()=>{
  const frames:Record<string,number>[]=[],until=performance.now()+2200;
  do{
   frames.push(window.__chacha.session!.renderer.debug().drummer as Record<string,number>);
 ...
 for(const part of ['head','tail','armLeft','armRight','backLeft','backRight','scarf','earLeft','earRight'])expect(new Set(frames.map(f=>f[part])).size,part).toBeGreaterThan(1);
 // Friends join as the festival gauge rises; the puppet keeps its joints.
