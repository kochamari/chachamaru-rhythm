...
音ズレ合わせ（#/sync）：選んだ出力経路で一定の拍（700ms間隔×20打、木の音）を鳴らし、遊ぶときと同じ入力で合わせて叩く。
各打撃をゲームと同じClockBridgeの対応で「最初の拍からのms」に直し、拍からのずれを−150〜550msの範囲で読む。
最初4打は除外し、各拍の最初の打撃だけを使い、中央値から大きく外れた打撃を除いて中央値とMAD（ばらつき）を表示。
8拍未満なら結果を出さない。中央値をその出力のaudioDelayMsにし（inputLagMsは0へ）、「元に戻す」を用意。
続けて小さなレーン（同じ対応・同じ補正）で試し叩きと±5/±10msの微調整ができる。
それで音声遅延と人間の反応と入力遅延を一意に分離できたとは説明しない（「叩き方のくせも含みます」と表示）。
測定と演奏の時計の方式をそろえるため、どちらも出力時計（getOutputTimestamp）が0を返す間は最大0.5秒待ってから時計を作る。
