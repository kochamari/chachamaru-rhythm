...
export interface Settings {bgm:number;hit:number;effect:number;ui:number;midiHit:number;effects:'standard'|'reduced'|'off';audioDelayMs:number;inputLagMs:number;visualAdvanceMs:number;profile:string;inputMode:InputMode;noteLabels:boolean;fastLate:boolean;festival:boolean;midi:{don:{note:number;channel:number}|null;ka:{note:number;channel:number}|null;velocity:number};calibrationProfiles:Record<string,{audioDelayMs:number;inputLagMs:number;visualAdvanceMs:number}>;/** Note scroll speed multiplier (visual only). Optional for older saves. */scrollSpeed?:number;/** Stroke sound set. Optional for older saves. */hitSound?:'taiko'|'pop'|'wood';/** Vibrate on pad taps (phones). Optional for older saves; on when missing. */haptics?:boolean}
